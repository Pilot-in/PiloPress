...
/**
 * Display a list of the page's layouts
 * in a HTML widget, if requested
 *
 * @return string
 */
function pip_display_layouts_summary() {
    ...
    if ( acf_maybe_get_GET( 'recettage', false ) !== false || getenv( 'PILO_RECETTAGE' ) === 'ON' ) {
        ...
        <script>
            document.addEventListener("DOMContentLoaded", () => {
                ...
                sommaire.classList.add("pip_sommaire");

                const label = document.createElement("label");
                label.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" fill="currentColor"><!--! Font Awesome Pro 6.3.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path d="M201.4 105.4c12.5-12.5 32.8-12.5 45.3 0l192 192c12.5 12.5 12.5 32.8 0 45.3s-32.8 12.5-45.3 0L224 173.3 54.6 342.6c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3l192-192z"/></svg>`;
                
                const button = document.createElement("input");
                button.id = label.htmlFor = "pip-sommaire";
                button.type = "checkbox";

                sommaire.append(button);
                sommaire.append(label);
                
                const sectionsAsNodes = document.querySelectorAll("body > section[id]");
                const sections = [];
                sectionsAsNodes.forEach(section => sections.push(section.id));
                sections.sort();
                ...
                sections.forEach(section => {
                    ...
                    const text = document.createTextNode(section.replace(regex, ""));
                    link.href = `#${section}`;
        ...
        <style>
            .pip_sommaire {
                ...
                background: var(--pip-color-secondary-500);
                filter: drop-shadow(0 20px 13px rgb(0 0 0 / 0.03)) drop-shadow(0 8px 5px rgb(0 0 0 / 0.08));
            }
            .pip_sommaire:has(:not(input:checked)) { border-radius: 100% }
            .pip_sommaire:has(input:checked) { border-radius: 1rem }
            .pip_sommaire div {
                display: none;
                flex-direction: column;
            }
            .pip_sommaire input:checked ~ div {
                display: flex;
            }
            .pip_sommaire a {
                display: list-item;
            ...
            .pip_sommaire input { display: none }
            .pip_sommaire label {
                order: 2;
                place-self: end;
                width: 2rem;
            ...
            .pip_sommaire input:checked ~ label {
                transform: scaleY(-1);
            }
        </style>
        <?php
